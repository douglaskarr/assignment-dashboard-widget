...
class MTZ_Todo_Ajax {
	...
	private function verify() {
		...
		if ( ! is_user_logged_in() || ! current_user_can( 'edit_posts' ) ) {
			wp_send_json_error( array( 'message' => __( 'Not allowed.', 'assignment-dashboard-widget' ) ) );
		}
	}
	...
	public function save_task() {
		...
		if ( ! $title ) {
			wp_send_json_error( array( 'message' => __( 'Title required', 'assignment-dashboard-widget' ) ) );
		}

		if ( $id ) {
			if ( ! $this->can_access( $id ) ) {
				wp_send_json_error( array( 'message' => __( 'Unauthorized', 'assignment-dashboard-widget' ) ) );
			}
			wp_update_post(
				array(
					'ID'         => $id,
		...
		if ( ! $id || is_wp_error( $id ) ) {
			wp_send_json_error( array( 'message' => __( 'Could not save to-do.', 'assignment-dashboard-widget' ) ) );
		}
